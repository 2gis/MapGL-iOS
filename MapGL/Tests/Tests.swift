...
class Tests: XCTestCase {
	...
	private var map: MapViewProtocol!
	...
	func test_execute_js_on_add_marker_without_image() {
		...
		"123");
		"""
		self.map.addMarker(self.testMarkerWithoutImage())
	...
	func test_execute_js_on_add_marker_with_image() {
		...
		[2.0, 1.0],
		[120.0, 177.0],
		"data:image/png;base64,\(self.testMarkerImageBase64String)",
		[0, 0],
		"123");
		"""
		self.map.addMarker(self.testMarkerWithImage(anchor: .leftTop))
		XCTAssertEqual(self.jsExecutor.javaScriptString, expected)
		XCTAssertEqual(self.jsExecutor.invocationsCount, 1)
	}
